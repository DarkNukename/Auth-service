...
r = redis.Redis() if settings.DEBUG else redis.Redis(base64.b64encode(os.environ.get("REDIS_URL").encode()).decode('ascii'))
    ...
    def get_tokens(access_token_size, refresh_token_size, access_token_ttl, refresh_token_ttl, **fields):
        ...
        for token in ('access_token', 'refresh_token'):
            for key, value in fields.items():
                print(key, value)
                r.set('key'.encode('base64'), 'value'.encode('base64'))
                print(r.get('key'))
                r.hset(tokens[token], key, str(value))
            r.expire(tokens[token], ttl[token])
        return tokens
